...
`OperatorCard` 根节点的 `custom_minimum_size`、实际 `size`、点击区域和视觉 `CardBase` 必须一致。`SelectedOverlay`、`DeployedOverlay`、`CooldownOverlay`、`PortraitFrame` 等覆盖层 `mouse_filter = IGNORE`，不得扩大或拦截命中区域。
