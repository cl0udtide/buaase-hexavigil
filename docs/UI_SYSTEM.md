...
- 右侧：上方为“今晚敌情”模块，含敌人/波次预告和路线预览开关；中部为选中单位/干员/商店干员详情，含头像占位、属性、SP、技能说明、技能/撤退/购买按钮；右下固定为战场图例。
...
- 若边框装饰不适合九宫格，才拆成角点和四边分段素材；简单 1px-2px 边线优先用 `StyleBoxFlat` 程序化绘制。
...
| `frame_wave_route_toggle_base` | `WavePreviewPanel/RouteToggle` | 120x32 | 路线预览开关底，必须放在敌情模块内部标题行 |
...
| `frame_action_panel_base` | `ActionPanel/Base` | 520x150 | 白天上下文操作面板底板，用于探索、入夜、建筑操作 |
| `frame_action_button_base` | `ActionPanel/Button` | 150x44 | 行动按钮底板，不写文字图标 |

### 6.2.2 关键部件推荐层级

- `TopBar`
  `TopBarBase(frame_top_status_bar_base)`、`LeftStatusGroup`、`CenterTimeGroup`、`RightResourceGroup` 三段式分布。`RightResourceGroup` 内用多个 `ResourceItem(frame_resource_item_base)` 表示行动点、木材、石材、魔力、声望等，不再把所有资源挤进一个 `ResourceLabel`。
- `BuildPanel`
...
- `WavePreviewPanel`
  “今晚敌情”是独立 HUD 模块，不是 tooltip。结构为 `WavePreviewBase`、标题行、路线预览开关、敌人条目/路线警告/正文。路线开关必须位于模块标题行内部。
- `ActionPanel`
...
│  ├─ DeployDeck
│  └─ LegendPanel
├─ DetailLayer (Control, z_index 45)
│  └─ UnitDetailPanel
...
当前场景可以先保留这些节点为 `CombatHud` 直接子节点，但必须用 `z_index` 达成上述顺序。`UnitDetailPanel` 应高于 `LegendPanel`、`WavePreviewPanel`、`DeployDeck` 和 `BuildPanel`，避免从底部干员卡或商店预览打开时被挡住。脚本中已有 `%SettingsButton`、`%TopBar`、`%RelicStrip`、`%DeployDeckContainer`、`%UnitDetailPanel`、`%RelicPanel`、`%AudioSettingsPanel`、`%DragGhost` 等引用；重构时若移动节点，必须保留 `unique_name_in_owner` 或同步更新脚本引用。
...
└─ WavePreviewMargin
   └─ WavePreviewContent
      ...
      ├─ WavePreviewScroll
      │  └─ WaveSpawnCardsBox  # 运行时补齐，按刷怪点分组显示敌人芯片
      ├─ WaveWarningLabel      # 运行时补齐，显示路线异常/拆墙提示
      └─ WavePreviewLabel      # 旧文本兜底；V2 数据可用时隐藏
```

“今晚敌情”模块应放在右侧详情卡上方，并比当前版本略微上移，让标题、路线开关、关卡文案、敌人卡片和警告行形成一个完整面板。`WaveRouteToggle` 不得漂在模块外部。敌情模块与 `UnitDetailPanel` 共享右侧列宽；当详情卡打开时，敌情模块占上方较小高度，详情卡从它下方开始。
...
```text
ActionPanel
├─ PanelBase                   # frame_action_panel_base
└─ ContentMargin
   └─ MainVBox
      ├─ ModeLabel
      ├─ ActionButtonFlow
...
- “今晚敌情”模块上移到右侧详情上方，路线开关位于模块内部标题行。
