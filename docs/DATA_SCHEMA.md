...
| `attack_delivery` | `String` | 攻击命中形式；`instant` 即时命中，`projectile` 飞行物命中，未配置默认 `instant` |
| `projectile_scene_key` | `String` | 飞行物场景逻辑名，未配置默认 `projectile` |
| `projectile_speed` | `float` | 飞行物飞行速度 |
| `projectile_color` | `Array` | 飞行物着色 RGBA |
| `projectile_hit_radius` | `float` | 飞行物命中半径 |
| `projectile_texture_path` | `String` | 飞行物贴图路径 |
...
| `visual_key` | `String` | 敌人贴图逻辑名，默认按同名资源查找 |
...
- 每晚波数与档位由 `scripts/enemy/night_template_resolver.gd` 的 `WAVE_TIERS_BY_DAY` 维护（九天三幕，d3/d6/d9 为幕末 Boss 晚）：
...
| `min_day` | `int` | 可选，模板入池的最早天数门控；未配置默认 1。解析当晚计划时只有 `min_day <= 当前天数` 的模板进入对应档位池 |
...
**日程表**：`ACTIVE_COUNT_BY_DAY = {1: 2, 4: 3, 6: 4, 8: 5}`。清晨按当天天数找最近触发的门槛，从激活序中取前 N 个口作为基础活跃集。
...
- 每晚词缀数量由 `scripts/enemy/night_affix_service.gd` 的 `AFFIX_COUNT_BY_DAY = {1: 1, 4: 2, 7: 3}` 维护（三幕逐档加压：d1-3 抽 1、d4-6 抽 2、d7-9 抽 3，按 <= 当天的最大键阶梯取值）。第 1 夜虽抽 1，但词缀池内最早 `min_day` 为 2，过滤后实际为 0 条。
...
| `damage_type_filter` | `String` | 可选，仅影响指定伤害类型，例如 `physical`、`magic` |
...
- 当前根事件池共 9 类：黑市商人、战争赌局、走私商队、古代祭坛、雇佣兵营地、废弃军械库、魔力裂隙、塌方契约、开口赌约。`events.json` 顶层共 27 条记录（9 个根事件 + 选项跳转的结果事件，如 `_deal`/`_leave`/`_accept`/`_decline` 等）。所有选项结果事件都应设置 `hidden_in_map_pool: true`。
...
| 字段 | 类型 | 说明 |
|---|---|---|
| `generator` | `string` | 生成器开关，三档：`"terrain_first"`（**现行**，见 `map_generator.gd:79-86`）/ `"skeleton_v2"`（保留备选，骨架生成器全管线）/ `"legacy"`（旧 walker 管线，随时可切回） |
...
generator 开关说明：`"terrain_first"`（**现行**）走地形优先管线（自然地貌 → 浮动核心选址 → 均匀放口 + 连通 → 盖到 CellData → 资源/事件），`"skeleton_v2"` 走骨架生成器全管线（牌组 → 车道 → 长肉 → 修复 → mesa → 资源），`"legacy"` 走旧 walker 管线；`obstacle_ratio`、`terrain_cluster_*` 等旧算法键全部保留，既作回切配置，也是 skeleton_v2 重试耗尽后的兜底生成器所用。`generate` 返回字典在 skeleton_v2 下额外回传 `sectors`（gate_key → {card, pass_grade, anchor, aperture, ford}）与 `gen_report`（attempts / fallback / ledger / intrusion / blocked_ratio 等）两键；新增键由 generate() 返回，map_manager 仅消费四个旧键（cells / core_cell / spawn_cells / event_points），sectors/gen_report 当前无运行时消费方（夜晚播报链接入需另行铺管），terrain_first 回传简版 gen_report、legacy 路径回传两个空字典。

**现行 terrain_first 路径的有效调参范围（重要）**：terrain_first 只从 cfg 读取 `spawn_count`（口数）以及（经 archetype 加权抽取）`archetypes` 的 `id`/`weight`；其余键基本不参与。具体地：

- 气候参数（山地/高台/低洼占比、河流数、湿度等）走 `terrain_field.gd:31` 硬编码的 `CLIMATE_PRESETS`，按抽中的 archetype id 取预设，**不读 cfg**；`archetypes` 里除 id/weight 外的字段（deck / confluence / ratio_band）在此路径不消费。
- 核心地形函数（`TerrainField.classify`、`CorePicker.pick_core`、`MapLayout.place_gates`/`ensure_connectivity`）只接收宽高/种子/气候预设/口数，**不接收 cfg**；`map_layout.gd:33,43` 的放口与连通逻辑是 terrain-blind 的，不读地图调参。

因此下列键**仅 skeleton_v2 / legacy 路径消费**，对现行 terrain_first 无效：`sector_cards`、`archetypes`（deck/confluence/ratio_band 部分）、`mesa`、`pass`、`repair`、`economy`、`detour_cap`/`detour_floor`、`corridor_slack`、`lane_jitter_base`、`gate_slide_jitter`、`moisture_gradient_strength`、`day1_card_constraint`、`bias_cards_by_activation`，以及 legacy 专用的 `obstacle_ratio`、`terrain_cluster_*`。改这些键想影响现行地图前，请先确认 `generator` 已切到对应档位。
...
- `wave_templates.json[].groups[].lane` 取值 main/flank/any
