...
> **美术方向说明（2026-06-12 环境画风 v3）**：环境画风定稿为"**干净的赛璐璐战场环境**"（见 §0 画风宪章），以现有干员/敌人 sprite 为同台基准、明日方舟主线关卡地图为质感参照。更早的"整体绿色调"与"清新战术奇幻低饱和暗色"两版表述均废弃；项目内现有地图与建筑素材仍是旧风格产物，应按本文档整体重生成替换。生成工具为 gpt-image-2（网页端，支持附参照图）。`UI_ASSET_GENERATION_PROMPTS.md` 仍是 UI 镶边资产的基准，环境不跟随 UI 的暗色低饱和，靠同一套功能点缀色保持亲缘。
...
## 0. 画风宪章与生成工作流（2026-06-12 定稿）

### 0.1 画风宪章

环境画风一句话：**干净的赛璐璐战场环境**——几何化体块、清晰的 2-3 阶赛璐璐影调、统一光向、颜色化阴影、明确的明度层级。质感参照明日方舟主线关卡地图；它必须和现有干员/敌人 sprite（高饱和赛璐璐、干净描边）同台和谐：环境是舞台，角色是主角。

五条可执行定义（写提示词、做验收都按这五条）：

1. **赛璐璐影调**：每个体块 2-3 阶色阶、影界干脆；禁止糊成一片的写实渐变、噪点质感和"整体灰蒙蒙"。
2. **统一左上暖光**：所有地形与建筑同一光向；阴影是偏冷（蓝紫倾向）的颜色化阴影，不是简单压暗。
3. **明度/饱和层级**：地表最沉最安静 → 地形特征（山/水/高台）中等 → 建筑最亮最饱和但仍低于单位。一眼能读出"什么重要"。
4. **几何化体块 + 干净剪影**：形体棱角分明、面片清楚（平顶必须读成平面，不要弧形大明暗）；细节集中在受光面与边缘，中心留白可叠单位。
5. **统一环境色板**：暖绿赭地表、青蓝水域、暖石色岩体、暖木+石材建筑；昼夜与氛围由代码全局调色，资产一律画"白天标准光"。

### 0.2 参照图工作流（每轮必带）

gpt-image-2 网页端生成时，每一轮都附带以下参照图：

- `assets/sprites/units/blaze/idle/blaze_idle_000.png`（干员风格基准）
- `assets/sprites/enemies/hound/idle/hound_idle_000.png`（敌人风格基准）
- 风格锚定稿后：再附 `assets/map/CommandMap/raw/style_anchor_sheet.png`（环境自身的风格锁，见 0.3）

并在该轮提示词末尾追加一句：

```text
附图是本游戏的角色 sprite（以及已定稿的环境资产）。生成的环境必须与它们同台和谐：同样干脆的赛璐璐影调与干净边缘，但饱和度低于角色——环境是舞台，角色站上去是主角。
```

### 0.3 风格锚流程（先锚后批，避免批量返工）

1. **第 0 轮只生成 3 个资产**：`tile_plain` + `tile_mountain`（取 §4 对应条目）+ `lumber_station`（取 §10.2 对应条目），拼成一轮。
2. 实装后游戏内截图（地块平铺 + 建筑 + 干员同框）交人工验收。
3. 验收通过的源图存为 `assets/map/CommandMap/raw/style_anchor_sheet.png`，此后每一轮生成都把它作为参照图附带。
4. 任何一轮与锚不一致（影调/光向/色板跑偏）直接废弃重生成，不要试图用后期调色救。

### 0.4 与代码侧合成层的分工

"立体感/不扁平"的另一半由代码完成，提示词不要把这些烘进贴图：

- **格子外的接触阴影**：建筑/单位落格后的投影由引擎统一绘制；keyed 资产自身不带投出格子的影子（地块内部的形体阴影属于资产本体，要画）。
- **地块间过渡**（山脚裙边、水岸线）：后续 overlay 资产/代码处理，基础地块保持独立完整。
- **昼夜与氛围**：全局调色 shader 负责；资产不要画夜景、不要自带氛围滤镜。

## 1. 全局提示词

每一轮都先复制本段，再追加对应批次提示词（参照图按 §0.2 附带）。

```text
我们要为一个 Godot 俯视塔防游戏生成地图地块源图。游戏中心是可读性很高的方形网格地图，四周是轻薄战术 HUD。地图必须是第一视觉层级，并与新版 UI 资产属于同一视觉家族。

整体风格：
干净的赛璐璐战场环境（以附图角色 sprite 与风格锚为基准）：几何化体块、清晰剪影、每个形体 2-3 阶干脆的影调分阶、统一左上暖光、偏冷蓝紫的颜色化阴影。地表是整张地图最安静的层——明度沉稳、细节克制，给单位、建筑与 UI 高亮让出对比空间；山地、水域等特征格允许更明确的体块和色彩，但环境饱和度始终低于角色。请根据每种地形的用途自由设计造型与装饰节奏；同一张源图内保持同一视觉家族。不要写实照片、PBR 材质贴图、噪点质感、厚重战争沙盘、糖果色卡通，也不要发灰发闷的整体压暗——画面要透气，层级靠明度对比而不是靠变暗。
...
如果模型倾向画得太亮太糖果，请改为：clean cel-shaded environment tiles, saturation clearly below the character sprites, restrained accents, fresh but calm。
如果模型倾向画得发灰发闷，请改为：clear value hierarchy, warm top-left key light, luminous lit planes, colored cool shadows — do not darken everything uniformly。
...
如果模型倾向画成写实照片或噪点质感，请改为：clean cel-shaded hand-painted game tile, 2-3 tone steps per volume, crisp shadow edges, no PBR material, no photo texture, readable at 64x64。
如果模型倾向画得过于扁平，请改为：faceted stylized terrain volumes with crisp cel shading, single top-left warm light, cool-tinted cast shadows inside the tile, top-down orthographic。
```
...
| 资产 key | 当前文件 | 用途 | 视觉目标 |
|---|---|---|---|
| `tile_plain` | `tile_plain.png` | 默认可行走/可建造平地 | 最安静的暖绿赭地表，明度沉稳不发灰，中心干净 |
...
| `tile_water` | `tile_water.png` | 水域阻挡 | 青蓝水面 2-3 阶分层，岸线干脆 |
...
```text
请生成一张地图地块源图，纯色背景 #FF00FF，包含 5 个独立正方形俯视地块，按从左到右排列。每个地块建议 256x256，最终会缩放到 64x64。风格按画风宪章：干净的赛璐璐战场环境、统一左上暖光、偏冷的颜色化阴影、几何化体块（以附图角色与风格锚为基准）。不要文字、数字、角色、建筑、UI 边框、粗网格线、照片材质、噪点质感、半透明像素。

1. tile_plain：默认平地。整张地图最安静的层：暖绿赭地表、明度沉稳但不发灰，可有大块干脆的浅色草痕/土面色斑和极少量两阶小草簇、碎石；中心干净，适合放置单位或建筑。不要写实草皮贴图，不要鲜绿草坪，不要花田，不要大图案，不要碎噪点。
2. tile_plain_alt：平地变化格。与 tile_plain 同族同明度，只改变色斑与点缀的布局，远看仍像平地。不要让它像资源点或阻挡物。
3. tile_hidden：未探索迷雾格。暗雾遮蔽，隐约有地形轮廓但信息被遮住，不纯黑，不画问号。
4. tile_mountain：山地阻挡。2-3 个棱角分明的岩石体块：面片化的受光面、干脆的影界、偏冷的暗部，左上暖光，剪影一眼读出不可通行。不要只画石头纹理平面，不要高耸写实山峰，不要等距山体，不要圆滚滚的卵石堆。
5. tile_water：水域阻挡。青蓝水面、干脆的岸线轮廓和 2-3 阶水色分层，少量干净的高光形状（赛璐璐式色块，不是写实反光）。不要只铺颜色纹理，不要刺眼亮色，不要海浪，不要写实波光。

整体必须是 top-down orthographic square cel-shaded game map tiles, single warm top-left light, cool colored shadows, faceted volumes, clear silhouettes, readable at 64x64, no perspective, no bevel frame, no photo texture, no noisy detail。
```
...
```text
请生成一张地图地块源图，纯色背景 #FF00FF，包含 2 个独立正方形俯视地块，按从左到右排列。每个地块建议 256x256，最终会缩放到 64x64。风格按画风宪章：干净赛璐璐环境、左上暖光、冷色化阴影、地标剪影清楚（以附图角色与风格锚为基准）。不要文字、数字、箭头、角色、敌人、建筑、UI 面板、照片材质、半透明像素。

1. tile_core：核心所在地。基底仍是安静的地表，中心有一个清晰但克制的核心晶体、能量基座或核心符印，是玩家要守护的目标地标。它是地图地标，不是 UI 按钮，不要厚边框，不要巨大水晶立绘，不要强外发光。
...
```text
请生成一张地图地块源图，纯色背景 #FF00FF，包含 4 个独立正方形俯视地块，按从左到右排列。每个地块建议 256x256，最终会缩放到 64x64。风格按画风宪章：干净赛璐璐环境、左上暖光、冷色化阴影，资源主体是棱角化小体块、一眼可辨（以附图角色与风格锚为基准）。不要文字、数字、资源图标徽章、UI 徽章、角色、建筑、半透明像素。
...
```text
请把色彩拉回环境色板：地表暖绿赭、水域青蓝、岩体暖石色、建筑暖木+石。如果太亮太糖果：降低饱和、收敛点缀，环境饱和度必须低于附图角色。如果发灰发闷：提高受光面明度与色温、保留干脆的影调分阶，不要整体压暗。
```
...
```text
请统一为单一左上暖光源：所有体块受光面在左上，阴影在右下且颜色偏冷（蓝紫倾向）。每个体块的影调收敛为 2-3 阶干脆分阶，影界清楚。不要多光源，不要均匀环境光，不要把阴影画成单纯加深的同色。
```

### 9.10 与角色不和谐

```text
附图是游戏内角色 sprite。请让环境与角色同台和谐：同样干脆的赛璐璐影调和干净边缘，但环境饱和度更低、对比更收敛——角色站在上面必须是画面主角，环境是舞台。不要让地形比角色更鲜艳或更抢眼。
```
...
整体风格：
按画风宪章的干净赛璐璐战场环境（以附图角色 sprite 与风格锚为基准）：建筑是地图上的小型几何体块实体——清楚的顶面/侧面面片、2-3 阶干脆影调、统一左上暖光、偏冷的暗部。建筑在明度层级中位于地表与地形特征之上、单位之下：允许比地块更亮更饱和的暖木+石材色，但不能抢过角色。请根据每个建筑的用途自由设计造型、比例、材质和细节，同一批建筑保持同一家族气质；不要复杂写实 3D 模型，不要噪点质感，不要发灰发闷。
...
- 需要落在地块的明度层级之上、单位之下：比地表亮而饱和，但不能抢过角色 sprite，也不能太像独立 UI 图标。
...
```text
请生成一张地图建筑 sprite 源图，纯色背景 #FF00FF，包含 6 个独立建筑，按从左到右排列。每个建筑最终会裁剪到 128x128 透明 PNG，并在 64x64 地图格上以约 72px 显示。风格按画风宪章：干净赛璐璐的小型几何体块建筑、左上暖光、冷色化暗部、暖木+石材色（以附图角色与风格锚为基准）。不要文字、数字、UI 边框、图标底板、徽章、卡片、地图地块背景、半透明像素。
...
```text
请生成一张地图建筑 sprite 源图，纯色背景 #FF00FF，包含 2 个独立建筑状态，按从左到右排列。最终每个资产裁剪为 128x128 透明 PNG。风格按画风宪章，与普通建筑同族（干净赛璐璐体块、左上暖光、冷色化暗部）。不要文字、数字、UI 边框、图标底板、徽章、地图地块背景、半透明像素。
...
```text
请生成一张通用损毁建筑地图 sprite，纯色背景 #FF00FF，只包含 1 个资产，最终裁剪为 128x128 透明 PNG。风格按画风宪章，与普通建筑同族（干净赛璐璐体块、左上暖光、冷色化暗部）。不要文字、数字、UI 边框、地面方块、火焰特效、角色、半透明像素。
...
```text
请改为干净赛璐璐影调的概括化塔防建筑：几何体块、2-3 阶分阶、左上暖光、冷色化暗部，轮廓清楚。不要写实建筑渲染、PBR 材质、复杂机械细节或强透视。
```
...
请生成一张地图地块源图，纯色背景 #FF00FF，包含 2 个独立正方形俯视地块，按从左到右排列。每个地块建议 256x256，最终缩放到 64x64。风格按画风宪章：干净赛璐璐环境、左上暖光、冷色化阴影、棱角化几何体块（以附图角色与风格锚为基准）。不要文字、数字、UI 边框、粗网格线、半透明像素。

1. tile_highland：高台地形格。棱角分明的平顶台地：**顶面必须读成一个干净的平面**（平缓的顶面色，不要画成弧形大明暗的圆丘/面包形），边缘是干脆的崖缘棱线；崖壁是面片化的垂直岩面，受光面与背光面分阶清楚，背光面颜色偏冷。剪影不要对称规整（允许一侧带半高的阶地式台阶增强地貌感）。色调用暖赭石色系，与 tile_mountain 的冷灰岩块、tile_plain 的绿赭地表都拉开明确色相差——远看一眼读出”这是一块高起来的可站立平台”，不是山、不是墙。顶面中心留白（之后有干员单位站在上面）。不要画梯子、旗帜、建筑、人物，不要画成悬浮岛或祭坛。
2. tile_ford：渡口浅滩格。在与 tile_water 同款的水面中，有一条由碎石/沙洲构成的可通行浅滩带，水纹在浅滩两侧断开，浅滩本体颜色接近平地地表表示可以走。不要画桥（木桥是建筑感）、不要画完整道路，保持自然浅滩感。

两格必须与本轮其余地块同一画风，铺在一起无违和。保持 top-down orthographic square cel-shaded game map tiles, faceted volumes, flat readable plateau top, readable at 64x64。
```
