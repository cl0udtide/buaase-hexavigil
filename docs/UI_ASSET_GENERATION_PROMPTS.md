...
- 主色为低饱和冷灰、深青灰、雾蓝灰，少量柔和浅金、浅琥珀、冷白、灰绿作为点缀。
- 青绿色只能作为极少量功能点缀，禁止大面积青绿、蓝绿霓虹和廉价页游感主色。
...
- 可缩放底板必须适合 Godot `StyleBoxTexture` 九宫格：四角不变形，上下边只横向延展，左右边只纵向延展，中心可拉伸或平铺。
- 不要生成需要整体等比缩放才能成立的复杂边框；不要让边框粗细依赖整张图缩放。
- 头像、建筑图标、技能图标、遗物图标必须夹在 backplate 和 frame 之间。frame 中心保持纯色背景 #FF00FF，方便后续抠成透明孔洞。
...
输出要求：
- 纯色背景，背景色固定为高对比洋红 #FF00FF，方便后续抠图；不要使用青绿色背景，不要透明棋盘格，不要渐变背景，不要场景背景。
- UI 主体、边缘柔光和半透明阴影都不能接近 #FF00FF，也不能把背景色混进边缘。
...
如果模型倾向画得太亮，请改为：暗色低饱和 UI 材质，base color around #18242A / #223238，accent around #C9A85C / #D8E4E6 but only subtle accents, avoid cyan-green as the dominant color。
...
- 如果某一轮出现背景不纯、洋红污染边缘、明显厚边框、臃肿、过亮、青绿主色、家具风、动态结构被画死，直接废弃该轮，使用第 29 节纠偏提示重新生成。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 9 个独立资产，按从左到右、从上到下排列，不要文字、数字、图标。
...
6. frame_icon_frame：通用图标覆盖框，约 96x96。中心保持 #FF00FF，后续抠透明。极薄边，不要厚框。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 7 个独立资产，按顺序排列，不要文字、数字。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 8 个独立资产。不要文字、数字、图标。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 8 个独立资产，不要文字、数字、建筑图标。
...
6. frame_build_icon_frame：建筑图标覆盖框，约 72x72。中心保持 #FF00FF，后续抠透明。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 11 个独立资产，不要文字、数字、人物、头像、职业图标。
...
9. frame_operator_portrait_frame：干员头像覆盖框，约 128x72。中心保持 #FF00FF，后续抠透明。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 9 个独立资产，不要文字、数字、头像、图标。
...
4. frame_unit_portrait_frame：单位头像覆盖框，约 128x128。中心保持 #FF00FF，后续抠透明，边框极薄。
...
8. frame_skill_icon_frame：技能图标覆盖框，约 72x72。中心保持 #FF00FF，后续抠透明。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 10 个独立资产，不要文字、数字、遗物图标。
...
7. frame_relic_icon_frame：遗物图标覆盖框，约 80x80。中心保持 #FF00FF，后续抠透明。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 2 个独立资产，不要文字、数字、图标、滑条。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 5 个独立资产，不要文字、数字、图标。
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 7 个独立资产，不要文字、数字、头像、图标。
...
```text
请生成一张 UI 图标资产源图，纯色背景 #FF00FF，包含 8 个图标，按 4x2 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张 UI 图标资产源图，纯色背景 #FF00FF，包含 12 个图标，按 4x3 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张 UI 图标资产源图，纯色背景 #FF00FF，包含 10 个图标，按 5x2 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张建筑图标资产源图，纯色背景 #FF00FF，包含 8 个图标，按 4x2 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张技能图标资产源图，纯色背景 #FF00FF，包含 8 个图标，按 4x2 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张技能图标资产源图，纯色背景 #FF00FF，包含 9 个图标，按 3x3 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张遗物图标资产源图，纯色背景 #FF00FF，包含 8 个图标，按 4x2 网格排列。不要文字、数字、外框或底板。
...
```text
请生成一张遗物图标资产源图，纯色背景 #FF00FF，包含 7 个图标，按 4+3 的网格排列。不要文字、数字、外框或底板。
...
```text
请只生成角色头像源图，纯色背景 #FF00FF。不要 UI 边框，不要卡牌，不要文字数字。头像构图要能放入 128x128 或 128x72 的头像窗口，人物边缘干净，光照柔和，低饱和轻微奇幻风。导出后头像位于 backplate 与 frame 之间。
```
...
如果生成结果有背景残边、抠图脏边或边缘混入色键，使用下面提示词：

```text
上一版边缘混入了背景色，透明 PNG 会出现脏边。请重新生成同一批资产：
- 背景必须是完全纯色 #FF00FF，不要渐变、阴影、噪点或假透明棋盘格。
- UI 主体、边缘高光、阴影和柔光都不能使用接近 #FF00FF 的品红、紫红、粉紫。
- 资产之间留出更大纯色背景间距，不互相投影，不接触画布边缘。
- 所有半透明光晕必须贴近主体，不能铺满整张源图。
- 保持原来的裁剪顺序。
```

如果生成结果仍然有大面积青绿或廉价页游感，使用下面提示词：

```text
上一版青绿色面积太大，像廉价页游 UI。请重新生成同一批资产：
- 禁止大面积青绿、蓝绿霓虹和高饱和 cyan/teal 主色。
- 主体改为低饱和深冷灰、深青灰、雾蓝灰。
- 点缀只用少量浅金、琥珀、冷白、灰绿；青绿最多作为极小功能高光。
- 降低发光强度和颜色纯度，去掉霓虹边、宝石边和厚外框。
- 保持纯色背景 #FF00FF 和原来的裁剪顺序。
```

如果生成结果不适合九宫格拉伸，使用下面提示词：

```text
上一版不适合 Godot 九宫格拉伸。请重新生成同一批 frame/base 资产：
- 四个角必须简单稳定，不要复杂角饰跨入边线或中心区。
- 上下边只能包含可横向延展的细边、轻纹理或内阴影。
- 左右边只能包含可纵向延展的细边、轻纹理或内阴影。
- 中心区域必须干净，可拉伸或平铺，不要固定槽位、图标框、文字或图案焦点。
- 边框视觉厚度保持 1px-2px 级，不要依赖整张图等比缩放。
- 保持纯色背景 #FF00FF 和原来的裁剪顺序。
```
...
```text
请生成一张 UI 资产源图，纯色背景 #FF00FF，包含 7 个独立资产，按从左到右、从上到下排列，不要文字、数字、图标。
