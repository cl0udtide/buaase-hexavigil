...
> **状态（2026-06-12 维护）**：风格基准已换代——以现有干员/敌人 sprite（高饱和赛璐璐）为同台基准，环境侧见 `MAP_ASSET_GENERATION_PROMPTS.md` §0 画风宪章；抠图底统一 **#FF00FF**。本文档 §2 全局提示词已按新基准重写；**第 5 节起各批次提示词中残留的"低饱和半卡通"等旧措辞，以 §2 为准覆盖**，待特效实际开始生成时逐批细修。特效资产尚未开始生成。
...
```text
我们要为一个 Godot 俯视塔防游戏生成战斗特效源图。游戏角色是高饱和赛璐璐 Q 版 sprite（生成时附 1-2 张作参照图），环境是干净赛璐璐影调的战场地图。特效需要让玩家快速读出攻击方向、命中范围、伤害类型或状态变化，但不能遮挡单位、路径、血条、部署格和 UI。
...
整体风格：
- 干脆的赛璐璐手绘特效：形状概括、剪影清楚、2-3 阶明度分阶、边缘干净，与附图的高饱和赛璐璐角色同台和谐。
- 和 UI、地图材质、角色、怪物属于同一个游戏，不是写实电影特效，不是照片烟尘，不是 PBR 爆炸，不是霓虹粒子海。
- 读图优先：远看先读出方向和范围，近看再看到少量干净的能量形状（色块化，不是噪点）。
- 功能色语言：物理浅金/冷白、法术青蓝、治疗暖白绿、警示暗红。瞬时特效（命中/剑气）饱和度可接近角色；持续型状态环与范围边框必须收敛克制。不要大面积纯色平铺，不要荧光绿，不要浓黑烟雾。
- 特效可以有轻微发光，但发光必须贴近主体，不要铺满整张图，不要形成大面积半透明雾。

网页端生图与抠图硬约束：
- 源图背景必须是纯色 #FF00FF，方便后续批量抠图；不要透明棋盘格，不要渐变背景，不要场景背景。
- 特效主体不要使用接近 #FF00FF 的洋红/品红色，尤其边缘、烟雾、光晕不能有洋红污染。需要粉紫色能量时，请偏向更冷的蓝紫或更暖的绯红，避免和背景色混在一起。
- 每个资产之间留足 #FF00FF 纯色间距，不要互相接触，不要互相投影。
...
- 每个序列帧从左到右播放：起势、展开、峰值、衰减、消散。帧之间用 #FF00FF 间隔，不画边框线。
...
如果模型倾向画得太亮太炸，请改为：clean cel-shaded VFX matching the attached character sprites, crisp 2-3 tone energy shapes, soft limited glow, readable but restrained, no neon particle storm。
如果模型倾向画成 UI 图标，请改为：in-world battle effect sprite, no icon frame, no badge, no button, no emblem, no decorative border。
如果模型倾向让背景难抠，请改为：flat solid chroma key background #FF00FF only, no background haze, no shadow touching background, effect colors clearly different from the background。
```
...
- 抠图时把 `#FF00FF` 转为透明。检查边缘不能残留洋红描边；必要时重新生成，而不是用过强羽化把特效边缘磨糊。
...
```text
请生成一张战斗命中特效序列帧源图，纯色背景 #FF00FF。画面包含 4 行特效，每行是 6 帧横向序列，从左到右播放。每帧建议 192x192 或 256x256，帧与帧之间留足 #FF00FF 间隔，不画边框线。最终会抠成透明 PNG，并在 Godot 中作为短暂命中特效播放。不要文字、数字、角色、敌人、地图格、UI 图标、徽章、按钮、照片烟尘。

1. impact_physical_small_strip：普通物理命中。冷白的小型冲击火花，少量碎线和短弧，第一帧很小，第三帧达到峰值，第六帧基本消散。不要大爆炸，不要血液，不要写实火花。
2. impact_arts_small_strip：法术命中。低饱和蓝白/紫蓝能量爆散，小型魔法碎光和短暂环形波。不要大魔法阵，不要霓虹蓝，不要贴近背景 #FF00FF 的洋红边缘。
3. impact_heal_small_strip：治疗反馈。柔和金白与很低饱和灰绿光点向上或向外扩散，温和、干净、正向。不要纯绿荧光，不要医疗 UI 图标，不要十字按钮，不要和背景 #FF00FF 混色。
...
```text
请生成一张战斗投射物源图，纯色背景 #FF00FF，包含 4 个独立投射物 sprite，按从左到右排列。每个投射物建议 256x96 或 256x128，主体沿 X 轴朝右，方便在 Godot 中旋转到飞行方向。不要文字、数字、角色、敌人、地图背景、UI 边框、按钮、徽章、完整插画。

1. projectile_arrow：弓弩/狙击投射物。细长箭矢或冷白弹道光，右侧为尖端，左侧有短而克制的运动尾迹。颜色冷白、浅金、灰蓝。不要真实枪弹，不要巨大箭头 UI，不要过长拖尾。
2. projectile_arts_orb：法术弹。小型蓝白/紫蓝能量球，右侧运动方向清楚，左侧短尾迹。不要使用接近背景 #FF00FF 的洋红边缘，不要大魔法阵。
...
```text
请生成一张真银斩/剑气特效源图，纯色背景 #FF00FF。画面包含 2 行特效：第 1 行是 8 帧横向剑气序列，第 2 行是 6 帧横向命中火花序列。帧与帧之间留足纯色背景，不画边框线。不要文字、数字、角色、敌人、地图格、攻击范围格、UI 图标、徽章、按钮。
...
```text
请生成一张通用范围边框材质源图，纯色背景 #FF00FF。这些资产不会铺满每个地图格，而是由 Godot 代码根据一组生效格子的外边界拼接成范围描边。请只生成边线、角点、端帽和小节点光，不要生成完整攻击范围形状，不要生成单格覆盖层，不要画地图格底图。不要文字、数字、箭头、角色、敌人、建筑、UI 按钮、厚边框。
...
```text
请生成一张持续类状态特效源图，纯色背景 #FF00FF。画面包含 4 行特效，每行是 8 帧横向循环序列。每帧建议 192x192 或 256x256，最终会叠在单位脚下或身上，不能遮挡角色主体。不要文字、数字、角色、敌人、地图格、UI 图标、徽章、按钮。
...
```text
请生成一张通用状态、领域和死亡反馈特效源图，纯色背景 #FF00FF。画面包含 8 行特效，每行是横向序列帧，从左到右播放。每帧建议 192x192 或 256x256，帧之间留足纯色背景。不要文字、数字、角色、敌人、建筑、地图格、UI 图标、徽章、按钮。
...
所有资产要保持低饱和半卡通战术奇幻风格。持续类特效必须低存在感，短促反馈必须中心锚点稳定。背景固定 #FF00FF，主体颜色不要接近背景色，边缘干净，方便抠图。
```
...
```text
请生成一张高优先级干员专属攻击特效源图，纯色背景 #FF00FF。画面包含 8 行特效，每行是横向序列帧。每行保持同一锚点、同一主体身份，不要每帧换设计。不要画干员、敌人、建筑、地图格、技能图标、文字或 UI。
...
```text
请生成一张领域、屏障和特殊干员持续效果源图，纯色背景 #FF00FF。画面包含 6 行循环序列帧，每行 8 帧，从左到右循环。它们会叠在单位、敌人或范围边界上，必须低存在感，不能遮住角色、血条或地图路径。不要文字、角色、敌人、建筑、地图格、UI 图标。
...
```text
请生成一张敌人与 Boss 专属特效源图，纯色背景 #FF00FF。画面包含护盾、死亡爆裂、分裂召唤、拆路重击、Boss 转阶段、Boss 入阶段范围爆发、Boss 重型投射物和冲击波。不要画完整敌人、Boss、建筑、地图格、UI、文字、血条或技能图标。
...
```text
请生成一张范围描边、技能启动和危险预警特效源图，纯色背景 #FF00FF。画面包含 8 行资产，每行横向序列帧，从左到右播放或循环。不要画完整攻击范围，不要画地图格，不要画角色、敌人、建筑、血条、文字或 UI 图标。范围类只做可平铺边线、角点、节点或短脉冲材质，方便代码沿外边界拼接。
...
所有范围素材都必须是外边界材质，不是范围地块贴图。边线要细、可重复平铺、中心对齐、亮度波动小。背景固定 #FF00FF，主体颜色不要接近背景色，边缘干净，方便抠图。
```
...
```text
请生成一张剩余干员攻击主特效源图，纯色背景 #FF00FF。画面包含 8 行资产，每个资产锚点稳定、方向明确、适合 Godot 俯视塔防中旋转、缩放或跟随目标播放。不要角色、敌人、建筑、地图格、技能图标、文字或 UI。
...
```text
请生成一张持续姿态、自回复和防御类干员特效源图，纯色背景 #FF00FF。画面包含 8 行资产，每行横向序列帧。不要角色、敌人、建筑、地图格、UI、文字、图标或完整范围覆盖层。
...
4. mountain_recover_pulse_strip：山自回复脉冲。脚下低存在感金白/灰绿恢复光，循环或低频播放，不要接近背景洋红。
...
```text
请生成一张敌人、投射物与环境补充特效源图，纯色背景 #FF00FF。画面包含 8 行资产，适合后续区分普通敌人远程、Boss 状态、建筑修复与敌人自回复。不要完整敌人、Boss、建筑、地图格、血条、UI、文字或图标。
...
敌方资产要比干员资产更粗粝一点，但仍是半卡通手绘、边缘干净、读图明确。背景固定 #FF00FF，主体颜色远离背景色，方便批量抠图。
```
...
- 检查背景是否为统一 `#FF00FF`。如果背景有压缩噪点、渐变、阴影或假透明棋盘格，不要继续裁剪。
- 抠图后分别放在深色平地、山地、水域、角色 sprite、怪物 sprite 上预览。边缘不能出现洋红残边。
...
```text
请重画为纯色 chroma key 背景，背景固定为 #FF00FF，整张图只有这一种背景色。不要渐变背景，不要透明棋盘格，不要背景烟雾，不要投影落在背景上。特效主体颜色必须和 #FF00FF 明显不同，边缘不能有青绿色污染。
```
...
```text
请减少半透明烟雾和大面积柔光，改用更清晰的手绘形状、短线、弧线和集中光点。主体边缘要清楚，背景保持纯 #FF00FF，方便干净抠成透明 PNG。不要让光晕融入背景。
```
