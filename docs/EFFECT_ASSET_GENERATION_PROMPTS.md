...
> **状态（2026-06-12 维护）**：风格基准已换代——以现有干员/敌人 sprite（高饱和赛璐璐）为同台基准，环境侧见 `MAP_ASSET_GENERATION_PROMPTS.md` §1 画风五条；抠图底统一 **#FF00FF**。本文档 §2 全局提示词已按新基准重写；**第 5 节起各批次提示词中残留的"低饱和半卡通"等旧措辞，以 §2 为准覆盖**，待特效实际开始生成时逐批细修。特效资产尚未开始生成。
...
- 如果某轮出现背景不纯、假透明棋盘格、写实烟尘、强霓虹、厚 UI 外框、文字、角色、地图格或帧间设计不一致，直接废弃该轮重新生成，重摇时口头指出违反哪条约定即可。
