...
由于仓库在 2026-04-23 才完成初始化，脚本会把 2026-04-23 及之前创建的首批 issue 视作 2026-04-20 已存在的初始工作量，并将这批 issue 的估点归一到总计 100。之后新增的 issue 会按标题和正文关键词估算 points，所以实际线可以体现开发中途新增任务导致的上升，以及任务关闭导致的下降。

## Estimate 标签

每个 issue 使用 `estimate:N` 标签记录估点，例如 `estimate:8`。这种格式便于脚本直接解析，也方便后续手工调整。
