...
- `set_mode_build(building_id)`
  ...
  行为：切换到建造模式并记录当前建筑。
  返回：无。
- `set_mode_deploy(operator_key)`
  输入：干员槽位 key。
  行为：切换到部署模式，后续点击地图时按当前朝向部署该干员槽位。
  返回：无。
- 选中场上单位
  行为：点击已部署单位所在格时，ActionPanel 记录该单位、显示技能信息并预览攻击范围。
- 取消选中场上单位
  行为：存在选中单位时，点击没有单位的地图格会清空选中单位并清除攻击范围预览。
- 释放技能 / 撤退
  行为：对当前选中单位调用 `UnitManager.try_cast_skill()` 或 `UnitManager.try_retreat_unit()`，并显示操作结果。
- `clear_mode()`
  输入：无。
  行为：清空当前模式。
  返回：无。
...
- `refresh_stock(stock_slots)`
  输入：商店槽位列表。每个槽位至少包含 `slot_index`、`unit_id`、`sold`。
  行为：刷新 5 格卡牌商店显示。
  返回：无。
...
- `refresh_owned_operators(operators)`
  输入：干员槽位字典列表。
  行为：按槽位刷新已拥有干员显示；同类多个槽位必须独立显示。
  返回：无。
- `refresh_operator_state(operator_key, state, remain_sec)`
  输入：干员槽位 key、状态名、剩余冷却时间。
  行为：刷新指定槽位状态；状态至少覆盖可部署、已部署、再部署冷却。
  返回：无。
- `set_visible_for_phase(phase)`
  输入：阶段枚举值。
  行为：根据当前阶段切换面板显隐。
...
- 作战 HUD 场景化容器，当前用于 `CombatSandbox`
...
| `deploy_limit_changed` | `current: int, max_value: int` | `RunState` | HUD、DeployPanel、CombatHud | 部署数量变化 |
| `owned_operators_changed` | `operators: Array[Dictionary]` | `RunState` | DeployPanel、CombatHud、CombatSandbox | 已拥有干员槽位变化 |
| `shop_stock_changed` | `stock_slots: Array[Dictionary]` | `ShopManager` | ShopPanel | 商店槽位库存变化 |
| `shop_action_result` | `action: StringName, result: Dictionary` | `ShopManager` | ShopPanel | 商店购买或刷新结果 |
...
| `request_deploy` | `operator_key: StringName, cell: Vector2i, facing: Vector2i` | UI | `UnitManager` | 请求部署指定干员槽位 |
...
| `unit_deployed` | `unit_runtime_id: int, operator_key: StringName, unit_id: StringName, cell: Vector2i` | `UnitManager` | HUD、DeployPanel、CombatHud、CombatSandbox | 干员槽位已部署 |
| `unit_removed` | `unit_runtime_id: int, reason: int` | `UnitManager` | HUD、DeployPanel、CombatHud、CombatSandbox | 单位离场 |
...
- 兼容购买单位：`EventBus.request_buy_unit.emit(unit_id)`
- 刷新商店：`EventBus.request_refresh_shop.emit()`
- 部署单位：`EventBus.request_deploy.emit(operator_key, cell, facing)`
...
调试场景可以在承载脚本中直接调用 Manager 接口，但 UI 组件本身仍应只发出信号。例如 `CombatHud` 发出干员卡片、暂停、倍速、技能和撤退信号，由 `CombatSandbox` 负责调用 `UnitManager`、`MapManager` 或修改 `SceneTree.paused`。
