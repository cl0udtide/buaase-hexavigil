...
var _tick_timer := 0.0
var _return_anchor_cells: Dictionary = {}
...
func _on_skill_start() -> void:
	_tick_timer = 0.0
	_return_anchor_cells.clear()
	var radius := _get_field_radius()
	if owner_unit.has_method("show_skill_range_outline"):
		owner_unit.show_skill_range_outline(&"shu_growth", _cells_in_radius(owner_unit.current_cell, radius), {
			"style": &"shu_growth",
	...
	_debug_log("技能启动：%s#%d 离离枯荣，领域治疗庇护并回拉敌人" % [
		owner_unit.unit_id,
...
func _on_skill_end() -> void:
	...
	if owner_unit != null and owner_unit.has_method("clear_skill_range_outline"):
		owner_unit.clear_skill_range_outline(&"shu_growth")
...
func _track_enemy_in_field(enemy: Node) -> void:
	...
	if not _return_anchor_cells.has(enemy_id):
		_return_anchor_cells[enemy_id] = enemy_cell
	if enemy.has_method("apply_move_speed_multiplier"):
		enemy.apply_move_speed_multiplier(&"shu_growth_field", float(owner_unit.cfg.get("skill_slow_multiplier", 0.6)), float(owner_unit.cfg.get("skill_shelter_duration", 1.4)))


func _pull_tracked_enemies() -> void:
	for raw_enemy_id in _return_anchor_cells.keys().duplicate():
		...
		if enemy == null or not is_instance_valid(enemy) or int(enemy.get("current_hp")) <= 0:
			_return_anchor_cells.erase(enemy_id)
			continue
		var anchor_cell: Vector2i = _return_anchor_cells[enemy_id]
		_pull_enemy_if_needed(enemy, anchor_cell)


func _pull_enemy_if_needed(enemy: Node, anchor_cell: Vector2i) -> void:
	var enemy_cell: Vector2i = enemy.get_current_cell()
	var pull_distance := int(owner_unit.cfg.get("skill_pull_distance", 3))
	if enemy_cell.distance_squared_to(anchor_cell) < pull_distance * pull_distance:
		return
	if enemy.has_method("apply_relocate_to_cell") and enemy.apply_relocate_to_cell(anchor_cell):
		_return_anchor_cells[int(enemy.get_runtime_id())] = anchor_cell


func _get_field_radius() -> int:
	return int(owner_unit.cfg.get("skill_growth_radius", 2))
