...
var _steps: Array[Dictionary] = [
	{"id": STEP_INTRO, "title": "守住六个夜晚", "body": "白天探索、采集、建造和招募；夜晚部署干员抵御敌人。教程会陪你完成第一天的关键操作。", "hint": "点击下一步开始。", "wait": false},
	...
	{"id": STEP_BLESSING, "title": "选择遗物", "body": "守住夜晚后会获得遗物选择。遗物会强化资源、建筑或干员，推动下一天的构筑。", "hint": "选择任意一件遗物。", "wait": true},
	{"id": STEP_DONE, "title": "教程完成", "body": "你已经走完第一天的完整循环。接下来继续探索、建造和招募，把防线撑到第六夜。", "hint": "点击完成关闭教程。", "wait": false}
]
...
func _start_if_first_run() -> void:
	...
	if run_state == null or int(run_state.day) != 1 or int(run_state.phase) != GameEnums.PHASE_DAY:
		return
...
func _on_unit_skill_cast(_unit_runtime_id: int, _operator_key: StringName, _unit_id: StringName) -> void:
