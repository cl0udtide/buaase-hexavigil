class_name UiTokens
extends RefCounted


const BASE_VIEWPORT := Vector2(1920.0, 1080.0)
const BREAKPOINT_COMPACT := 1500.0
const BREAKPOINT_NARROW := 1280.0

const EDGE_DESKTOP := 14.0
const EDGE_COMPACT := 10.0
const EDGE_NARROW := 8.0

const SPACE_2XS := 4.0
const SPACE_XS := 6.0
const SPACE_SM := 8.0
const SPACE_MD := 10.0
const SPACE_LG := 12.0
const SPACE_XL := 16.0

const RADIUS_SM := 4.0
const RADIUS_MD := 6.0

const BUTTON_H_LG := 56.0   # 主 CTA(菜单/结算)
const BUTTON_H_MD := 40.0   # 面板内动作按钮(锁定/刷新/购买/升星等)
const CHIP_H := 30.0        # 徽章/小签
const POPUP_BUTTON_HEIGHT := 44.0  # 地图弹窗/事件弹窗动作按钮统一高

const FONT_XS := 13  # 字号下限;13 以下禁止用于任何可读文本
const FONT_SM := 13
const FONT_MD := 15
const FONT_LG := 16
const FONT_XL := 22

const TOP_BAR_HEIGHT := 82.0
const TOP_BAR_HEIGHT_COMPACT := 76.0
const TOP_BAR_Y := 8.0
const SETTINGS_BUTTON_SIZE := 44.0
const SETTINGS_BUTTON_SIZE_COMPACT := 40.0
const SETTINGS_PANEL_SIZE := Vector2(420.0, 300.0)
const RELIC_STRIP_HEIGHT := 44.0
const RELIC_STRIP_HEIGHT_COMPACT := 40.0
const RELIC_STRIP_MIN_WIDTH := 260.0
const RELIC_STRIP_MAX_WIDTH := 1100.0
const RELIC_PANEL_SIZE := Vector2(900.0, 640.0)

const LEFT_PANEL_WIDTH := 320.0
const LEFT_PANEL_WIDTH_COMPACT := 304.0
const LEFT_PANEL_WIDTH_NARROW := 286.0
const LEFT_PANEL_MIN_HEIGHT := 360.0

const DETAIL_WIDTH := 384.0
const DETAIL_WIDTH_COMPACT := 342.0
const DETAIL_WIDTH_NARROW := 320.0
const DETAIL_TOP := 128.0
const DETAIL_MIN_HEIGHT := 260.0
const DETAIL_MIN_HEIGHT_COMPACT := 260.0
const DETAIL_MIN_HEIGHT_NARROW := 250.0
const DETAIL_PREFERRED_HEIGHT := 560.0
const DETAIL_PREFERRED_HEIGHT_COMPACT := 500.0
const DETAIL_PREFERRED_HEIGHT_NARROW := 460.0
const RIGHT_COLUMN_TOP_SHIFT := 48.0
const RIGHT_COLUMN_TOP_SHIFT_COMPACT := 44.0
const RIGHT_COLUMN_TOP_SHIFT_NARROW := 40.0
const RIGHT_COLUMN_TOP_BAR_GAP := SPACE_XS
const RIGHT_COLUMN_BOTTOM_GAP := SPACE_2XS

const WAVE_PREVIEW_HEIGHT := 156.0
const WAVE_PREVIEW_HEIGHT_COMPACT := 142.0
const WAVE_PREVIEW_HEIGHT_NARROW := 132.0
const WAVE_PREVIEW_MIN_HEIGHT := 104.0
const WAVE_PREVIEW_MAX_COLUMN_RATIO := 0.42

const LEGEND_SIZE := Vector2(244.0, 144.0)
const LEGEND_SIZE_COMPACT := Vector2(218.0, 118.0)
const LEGEND_SIZE_NARROW := Vector2(202.0, 104.0)

const DEPLOY_DECK_HEIGHT := 190.0
const DEPLOY_DECK_HEIGHT_COMPACT := 176.0
const DEPLOY_DECK_MIN_WIDTH := 520.0

const ACTION_PANEL_SIZE := Vector2(318.0, 76.0)
const ACTION_PANEL_SIZE_COMPACT := Vector2(304.0, 70.0)
const ACTION_PANEL_SIZE_NARROW := Vector2(286.0, 66.0)

const OPERATOR_CARD_SIZE := Vector2(164.0, 184.0)
const OPERATOR_CARD_COMPACT_SIZE := Vector2(148.0, 172.0)


static func edge_for_width(width: float) -> float:
	if width <= BREAKPOINT_NARROW:
		return EDGE_NARROW
	if width <= BREAKPOINT_COMPACT:
		return EDGE_COMPACT
	return EDGE_DESKTOP


static func is_compact(width: float) -> bool:
	return width <= BREAKPOINT_COMPACT


static func is_narrow(width: float) -> bool:
	return width <= BREAKPOINT_NARROW
