...
func _ready() -> void:
	AppTheme.apply(self)
	_bind_button("%BuildLumberButton", &"lumber_station")
	_bind_button("%BuildStoneButton", &"stone_quarry")
	_bind_button("%BuildManaButton", &"mana_extractor")
...
func _on_build_button_pressed(building_id: StringName) -> void:
	...
	if label != null:
		label.text = "Selected: %s" % String(building_id)
